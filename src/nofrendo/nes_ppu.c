...
static void ppu_buildpalette(ppu_t *src_ppu, rgb_t *pal)
{
	...
	for (i = 0; i < 64; i++)
	{
		rgb_t* tmp = &src_ppu->curpal[i + 64];
		rgb_t* tmp2 = &src_ppu->curpal[i + 128];
		src_ppu->curpal[i].r = tmp->r = tmp->r = pal[i].r;
		src_ppu->curpal[i].g = tmp->g = tmp->g = pal[i].g;
		src_ppu->curpal[i].b = tmp->b = tmp->b = pal[i].b;
	}

	for (i = 0; i < GUI_TOTALCOLORS; i++)
	{
		rgb_t* tmp = &src_ppu->curpal[i + 192];
		tmp->r = gui_pal[i].r;
		tmp->g = gui_pal[i].g;
		tmp->b = gui_pal[i].b;
	}
...
static void ppu_fakeoam(int scanline)
{
	...
	/* Check to see if sprite is out of range */
	if ((sprite_y > scanline)
		|| (sprite_y <= (scanline - sprite_height)) 
		|| (0 == sprite_y)
		|| (sprite_y > 240)
	)	return;
...
INLINE void draw_deadsprite(bitmap_t *bmp, int x, int y, int height)
{
	...
	uint8 *vid;
	uint8 colbuf[8] = {
		GUI_BLACK, GUI_BLACK, GUI_BLACK, GUI_BLACK,
